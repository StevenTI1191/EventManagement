...
<head>
    ...
    <style>
        ...
        .total { font-size: 22px; font-weight: 800; color: #FF2D55; }
        .divider { border: none; border-top: 1px solid #f3f4f6; margin: 20px 0; }
        .attach { display: inline-block; background: #fff5f7; border: 1px solid #ffd6de; border-radius: 10px; padding: 12px 18px; font-size: 13px; color: #b4123a; font-weight: 600; margin-bottom: 20px; }
...
<div class="wrapper">
    <div class="header">
        ...
        <h1>Penawaran Acara Anda 📩</h1>
        <p>Tim Event Marketing telah menyiapkan penawaran untuk Anda</p>
    </div>
    <div class="body">
        <p class="greeting">Halo, {{ $event->client?->nama_client ?? 'Bapak/Ibu' }}!</p>
        <p class="text">
            Terima kasih atas ketertarikan Anda bekerja sama dengan
            <strong>PT Laksamana Muda Bersatu</strong>. Berikut kami sampaikan penawaran untuk acara Anda.
            Rincian lengkap kami lampirkan dalam berkas PDF pada email ini.
        </p>
        ...
        <div class="card">
            ...
            <div class="card-row">
                ...
                <span class="card-value">
                    {{ $event->tgl_mulai_event ? \Carbon\Carbon::parse($event->tgl_mulai_event)->translatedFormat('d F Y') : '-' }}
                </span>
            </div>
            <div class="card-row">
                <span class="card-label">Waktu</span>
                <span class="card-value">{{ substr((string) $event->jam_mulai, 0, 5) }} – {{ substr((string) $event->jam_selesai, 0, 5) }} WIB</span>
            </div>
            <div class="card-row">
            ...
            <hr class="divider">
            <div class="card-row">
                <span class="card-label">Total Penawaran</span>
                <span class="card-value total">Rp {{ number_format((float) ($event->deal_harga_event ?? 0), 0, ',', '.') }}</span>
            </div>
        </div>

        <div class="attach">📎 Dokumen penawaran (PDF) terlampir pada email ini</div>

        <p class="text">
            Silakan tinjau penawaran ini. Bila berkenan, Anda dapat <strong>menerima</strong> atau
            <strong>menolak</strong> penawaran melalui portal klien kami. Setelah penawaran diterima,
            tim Finance akan menerbitkan invoice uang muka (DP 50%) untuk mengamankan tanggal acara Anda.
        </p>
        <p class="text">Kami tunggu kabar baik dari Anda. Terima kasih! 🙏</p>
    </div>
    <div class="footer">
        <p>© {{ date('Y') }} PT Laksamana Muda Bersatu · Email ini dikirim otomatis, mohon tidak membalas.</p>
    </div>
</div>
</body>
</html>
