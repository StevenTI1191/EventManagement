/**
 * Halaman Transaksi bersama Finance & Manajemen.
 *
 * Sebelumnya keduanya punya salinan sendiri: Manajemen tertinggal tanpa tab
 * Internal/Eksternal dan tanpa form sunting pembayaran maupun item, padahal
 * rutenya sudah ada. Satu komponen dipakai bersama supaya keduanya tidak
 * kembali melenceng; nama rutenya dibentuk dari prefiks peran.
 */
...
export default function TransaksiIndex({ Layout, prefix, auth, events, filters = {}, counts = {} }) {
    ...
    const gantiTipe = (tipe) => {
        ...
        setFilterData(fresh);
        router.get(route(`${prefix}.transaksi.index`), fresh, { preserveState: true, replace: true });
    };
    ...
    const applyFilter = (updated) => {
        ...
        router.get(route(`${prefix}.transaksi.index`), updated, { preserveState: true, replace: true });
    };

    const resetFilter = () => {
        const fresh = { tipe: tipeAktif, bulan: '', tahun: '', status: '', sort: 'tanggal', dir: 'desc', search: '' };
        setFilterData(fresh);
        router.get(route(`${prefix}.transaksi.index`), { tipe: tipeAktif }, { preserveState: true, replace: true });
    };
    ...
    const SortIcon = ({ field }) => {
        if (filterData.sort !== field) return <span className="ml-1 opacity-30">↕</span>;
        return <span className="ml-1">{filterData.dir === 'desc' ? '↓' : '↑'}</span>;
    };
    ...
    const formEditBayar = useForm({
        _method: 'PUT', nominal: '', tgl_bayar: '', keterangan: '', bukti_file: null,
    });

    const formEditItem = useForm({
        _method: 'PUT', tipe: 'Pengeluaran', nama_item: '', qty: 1, harga: '', keterangan: '',
    });
    ...
    const handleBayar = (e) => {
        e.preventDefault();
        formBayar.post(route(`${prefix}.transaksi.store`), {
            forceFormData: true,
    ...
    const handleItem = (e) => {
        e.preventDefault();
        formItem.post(route(`${prefix}.transaksi.item.store`), {
    ...
    const handleEditBayar = (e) => {
        e.preventDefault();
        formEditBayar.post(route(`${prefix}.transaksi.update`, editBayar.id_transaksi), {
            forceFormData: true,
    ...
    const handleEditItem = (e) => {
        e.preventDefault();
        formEditItem.post(route(`${prefix}.transaksi.item.update`, editItem.id_item), {
    ...
    const openEditBayar = (p) => {
        setEditBayar(p);
        formEditBayar.setData({ _method: 'PUT', nominal: p.nominal, tgl_bayar: p.tgl_bayar, keterangan: p.keterangan || '', bukti_file: null });
    };

    const openEditItem = (item) => {
        setEditItem(item);
        formEditItem.setData({ _method: 'PUT', tipe: item.tipe, nama_item: item.nama_item, qty: item.qty, harga: item.harga, keterangan: item.keterangan || '' });
    };
    ...
    return (
        <Layout>
            ...
            {modalBayar && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setModalBayar(null)}>
                    <div className="bg-white rounded-[2rem] w-full max-w-md shadow-2xl p-8" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-between mb-6">
                            ...
                            <button onClick={() => setModalBayar(null)} className="flex items-center justify-center w-8 h-8 rounded-full hover:bg-gray-100">
                        ...
                        <form onSubmit={handleBayar} className="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="date" className="w-full p-3 text-sm border border-gray-200 rounded-xl bg-gray-50"
                            ...
                            <div className="flex gap-3 pt-2">
                                <button type="button" onClick={() => setModalBayar(null)}
                                    className="flex-1 py-3 text-sm font-bold text-gray-500 border border-gray-200 rounded-xl hover:bg-gray-50">Batal</button>
            ...
            {/* MODAL TAMBAH ITEM */}
            {modalItem && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setModalItem(null)}>
                    <div className="bg-white rounded-[2rem] w-full max-w-md shadow-2xl p-8" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-between mb-6">
                            <div>
                            ...
                            <button onClick={() => setModalItem(null)} className="flex items-center justify-center w-8 h-8 rounded-full hover:bg-gray-100">
                                <X size={16} className="text-gray-500" />
                            </button>
                        </div>
                        <form onSubmit={handleItem} className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <input type="number" min="1" className="w-full p-3 text-sm border border-gray-200 rounded-xl bg-gray-50"
                                ...
                                <div>
                                    ...
                                    <RupiahInput placeholder="0" className="w-full p-3 text-sm border border-gray-200 rounded-xl bg-gray-50"
                            ...
                            {formItem.data.qty && formItem.data.harga && (
                                <div className="p-3 bg-gray-50 rounded-xl">
                                    ...
                                    <p className="text-sm font-extrabold text-gray-800">{formatRupiah(formItem.data.qty * formItem.data.harga)}</p>
                                </div>
                            )}
                            <div>
                                <label className="block mb-1 text-xs font-bold text-gray-600">Keterangan (Opsional)</label>
                                <input type="text" placeholder="Keterangan tambahan" className="w-full p-3 text-sm border border-gray-200 rounded-xl bg-gray-50"
            ...
            {editBayar && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setEditBayar(null)}>
                    <div className="bg-white rounded-[2rem] w-full max-w-md shadow-2xl p-8" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-between mb-6">
                            <h2 className="text-lg font-extrabold text-gray-900">Edit Pembayaran</h2>
                            <button onClick={() => setEditBayar(null)} className="flex items-center justify-center w-8 h-8 rounded-full hover:bg-gray-100">
                                <X size={16} className="text-gray-500" />
                            </button>
                        </div>
                        <form onSubmit={handleEditBayar} className="space-y-4">
                            <div>
                                <label className="block mb-1 text-xs font-bold text-gray-600">Nominal</label>
                                <RupiahInput className="w-full p-3 text-sm border border-gray-200 rounded-xl bg-gray-50"
                                    value={formEditBayar.data.nominal} onChange={v => formEditBayar.setData('nominal', v)} />
                            </div>
                            <div>
                                <label className="block mb-1 text-xs font-bold text-gray-600">Tanggal Bayar</label>
                                <input type="date" className="w-full p-3 text-sm border border-gray-200 rounded-xl bg-gray-50"
                                    value={formEditBayar.data.tgl_bayar} onChange={e => formEditBayar.setData('tgl_bayar', e.target.value)} />
                            </div>
                            <div>
                                <label className="block mb-1 text-xs font-bold text-gray-600">Keterangan</label>
                                <input type="text" className="w-full p-3 text-sm border border-gray-200 rounded-xl bg-gray-50"
                                    value={formEditBayar.data.keterangan} onChange={e => formEditBayar.setData('keterangan', e.target.value)} />
                            </div>
                            <div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setEditItem(null)}>
                    <div className="bg-white rounded-[2rem] w-full max-w-md shadow-2xl p-8" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-between mb-6">
                            ...
                            <button onClick={() => setEditItem(null)} className="flex items-center justify-center w-8 h-8 rounded-full hover:bg-gray-100">
                                <X size={16} className="text-gray-500" />
                            </button>
                        </div>
                        <form onSubmit={handleEditItem} className="space-y-4">
                            <div>
                                <label className="block mb-1 text-xs font-bold text-gray-600">Tipe</label>
                                <select className="w-full p-3 text-sm border border-gray-200 rounded-xl bg-gray-50"
                                    value={formEditItem.data.tipe} onChange={e => formEditItem.setData('tipe', e.target.value)}>
                                    <option value="Pengeluaran">Pengeluaran</option>
                                    <option value="Pemasukan">Pemasukan</option>
                                </select>
                            </div>
                            <div>
                                <label className="block mb-1 text-xs font-bold text-gray-600">Nama Item</label>
                                <input type="text" className="w-full p-3 text-sm border border-gray-200 rounded-xl bg-gray-50"
                                    value={formEditItem.data.nama_item} onChange={e => formEditItem.setData('nama_item', e.target.value)} />
                            </div>
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    <label className="block mb-1 text-xs font-bold text-gray-600">Qty</label>
                                    <input type="number" min="1" className="w-full p-3 text-sm border border-gray-200 rounded-xl bg-gray-50"
                                        value={formEditItem.data.qty} onChange={e => formEditItem.setData('qty', e.target.value)} />
                                </div>
                                <div>
                                    <label className="block mb-1 text-xs font-bold text-gray-600">Harga</label>
                                    <RupiahInput className="w-full p-3 text-sm border border-gray-200 rounded-xl bg-gray-50"
                                        value={formEditItem.data.harga} onChange={v => formEditItem.setData('harga', v)} />
                                </div>
                            </div>
                            {formEditItem.data.qty && formEditItem.data.harga && (
                                <div className="p-3 bg-gray-50 rounded-xl">
                                    <p className="text-xs text-gray-400">Total</p>
                                    <p className="text-sm font-extrabold text-gray-800">{formatRupiah(formEditItem.data.qty * formEditItem.data.harga)}</p>
                                </div>
                            )}
                            <div>
                                <label className="block mb-1 text-xs font-bold text-gray-600">Keterangan</label>
                                <input type="text" className="w-full p-3 text-sm border border-gray-200 rounded-xl bg-gray-50"
                                    value={formEditItem.data.keterangan} onChange={e => formEditItem.setData('keterangan', e.target.value)} />
                            </div>
                            <div className="flex gap-3 pt-2">
            ...
            <div className="p-5 mb-6 bg-white border border-gray-100 shadow-sm rounded-2xl">
                ...
                {(filterData.bulan || filterData.tahun || filterData.status || filterData.search) && (
                    <div className="flex flex-wrap gap-2 mt-3 pt-3 border-t border-gray-100">
                        ...
                        {filterData.bulan && (
                            <span className="px-2.5 py-1 text-xs font-bold bg-pink-50 text-[#FF2D55] rounded-full">
                                {bulanList.find(b => b.value === filterData.bulan)?.label}
                            </span>
                        )}
                        {filterData.tahun && (
                            <span className="px-2.5 py-1 text-xs font-bold bg-pink-50 text-[#FF2D55] rounded-full">
            ...
            <div className="overflow-hidden bg-white border border-gray-100 shadow-sm rounded-2xl">
                <table className="w-full">
                    ...
                    <tbody className="divide-y divide-gray-50">
                        {events.data && events.data.length > 0 ? events.data.map((event, index) => (
                                                        ...
                                                        <table className="w-full mb-4">
                                                            <thead>
                                                                <tr className="text-left border-b border-gray-200">
                                                                    ...
                                                                    <th className="pb-2 text-xs font-bold text-gray-400">Ditambahkan oleh</th>
                                                            ...
                                                            <tbody className="divide-y divide-gray-100">
                                                                {event.pembayarans.map((p) => (
                                                                    <tr key={p.id_transaksi}>
                                                                        ...
                                                                        <td className="py-2 text-xs text-gray-500">{p.nama_pegawai || '-'}</td>
                                                                        <td className="py-2">
                                                                            {p.bukti_file
