            ...
            <div className="min-h-screen bg-paper">
                    ...
                    <div className="flex items-center justify-between max-w-3xl mx-auto">
                        ...
                        <div className="flex items-center gap-2.5">
                            ...
                            <div>
                                ...
                                <p className="text-[10px] text-muted-2 leading-none mt-0.5">Profil Saya</p>
                            </div>
                        </div>
                ...
                <div className="max-w-3xl px-6 py-10 mx-auto">
                    ...
                    <div className="flex items-center gap-4 mb-8">
                        <div className="flex items-center justify-center w-16 h-16 sm:w-20 sm:h-20 text-xl sm:text-2xl font-black text-white bg-gold rounded-full flex-shrink-0">
                            {user?.nama_client?.substring(0, 2).toUpperCase() ?? '??'}
                        </div>
                        <div className="min-w-0">
                            <h1 className="text-xl font-black text-ink truncate sm:text-2xl">{user?.nama_client}</h1>
                            <p className="text-sm text-muted truncate">{user?.email_client}</p>
                            {user?.perusahaan_client && (
                                <p className="text-xs text-gold/70 mt-0.5 truncate">🏢 {user.perusahaan_client}</p>
                            )}
                            {/* Google-linked badge */}
                            {user?.has_google && (
                                <span className="inline-flex items-center gap-1 mt-1.5 px-2 py-0.5 text-[10px] font-bold bg-blue-500/10 text-info border border-blue-500/20 rounded-full">
                    ...
                    {/* Flash */}
                    {flash?.success && (
                        <div className="flex items-center gap-3 p-4 mb-6 bg-ok-bg border border-ok/30 rounded-xl">
                            <CheckCircle size={18} className="text-ok flex-shrink-0" />
                            <p className="text-sm font-bold text-ok">{flash.success}</p>
                        </div>
                    )}
                    {flash?.warning && (
                        <div className="flex items-center gap-3 p-4 mb-6 border bg-gold-soft border-gold-2 rounded-xl">
                            <span className="text-lg flex-shrink-0">📋</span>
                            <p className="text-sm font-bold text-gold-dim">{flash.warning}</p>
                        </div>
                    )}
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                                    ...
                                    <div>
                                        ...
                                        <div className="relative">
                                            <Phone size={15} className="absolute text-muted-2 left-3 top-3.5" />
                        ...
                        {/* Ganti Password */}
                        <div className="p-6 bg-surface border border-line rounded-2xl">
                            <h2 className="flex items-center gap-2 mb-1 text-sm font-extrabold text-ink">
                                ...
                                {has_password && (
                                    <div className={`transition-all duration-200 ${isChangingPassword ? 'opacity-100' : 'opacity-50'}`}>
                                        <label className={labelClass}>
                                            Password Saat Ini {isChangingPassword && <span className="text-danger">*</span>}
                                        </label>
                                        <div className="relative">
                                        ...
                                        {errors.current_password && (
                                            <p className="mt-1 text-xs text-danger">⚠ {errors.current_password}</p>
                                        )}
                                    </div>
                                )}
                        ...
                        <div className="flex gap-4">
                            <Link href={route('client.dashboard')}
                                className="flex-1 py-3 text-sm font-bold text-center text-muted transition-colors border border-line rounded-xl hover:bg-paper">
                                Batal
                            </Link>
