/**
 * Daftar appointment, dipakai Event Marketing & Manajemen.
 * Diambil dari versi Manajemen karena punya pencarian & hitungan
...
export default function ManajemenAppointmentIndex({ Layout, routes = {},  auth, appointments, counts, filters }) {
        ...
        <Layout>
            ...
            <div className="space-y-4">
                {appointments.data?.length > 0 ? appointments.data.map(apt => (
                    <div key={apt.id}
                        onClick={() => routes.show && router.visit(route(routes.show, apt.id))}
                        title={routes.show ? 'Klik untuk membuka detail appointment' : undefined}
                        className={`p-5 bg-white border border-gray-100 shadow-sm rounded-2xl hover:shadow-md transition-shadow ${
                            routes.show ? 'cursor-pointer hover:border-gray-200' : ''
                        }`}>
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <div className="flex flex-wrap items-center gap-3 mb-3">
                                    <h3 className="text-base font-extrabold text-gray-900">
                                        {apt.jenis_event}
                                        {/* Penanda "negosiasi penawaran" sengaja tidak ada di sini:
                                            daftar ini memang menyaring habis pertemuan tersebut
                                            (whereDoesntHave('negosiasi')), jadi penandanya tidak
                                            akan pernah muncul. Pembahasan penawaran dibuka dari
                                            menu Negosiasi Klien. */}
                                    </h3>
