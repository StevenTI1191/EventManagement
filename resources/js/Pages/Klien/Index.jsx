...
export default function EMClientIndex({ Layout, routes = {}, canEdit = true,  clients, filters, sumber = 'Mandiri', jumlah = {} }) {
    ...
    const gantiTab = (key) => {
        setSearchTerm('');
        router.get(route(routes.index), { sumber: key }, { preserveState: true, replace: true });
    };
    ...
    const confirmDeleteClient = () => {
        if (deleting) return;
        setDeleting(true);
        router.delete(route(routes.destroy, deleteClient.id), {
            preserveScroll: true,
                            ...
                            <tr key={client.id} className="transition-colors hover:bg-gray-50/60">
                                ...
                                <td className="px-6 py-4">
                                    <div className="flex items-center gap-2">
                                        <Link
                                            href={route(routes.show, client.id)}
                                        ...
                                        {canEdit && (
                                            <>
                                                ...
                                                <Link
                                                    href={route(routes.edit, client.id)}
